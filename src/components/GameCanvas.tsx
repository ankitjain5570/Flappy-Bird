...
export const GameCanvas = forwardRef<GameEngine, { events: EngineEvents; muted: boolean; interactive?: boolean }>(({ events, muted, interactive = true }, ref) => {
  ...
  useEffect(() => { const next = new GameEngine(canvas.current!, events, muted); engine.current = next; setInstance(next);
    if (import.meta.env.DEV) (window as unknown as { __engine?: GameEngine }).__engine = next; const resize = () => next.resize(); window.addEventListener('resize', resize); return () => { window.removeEventListener('resize', resize); next.destroy(); if (engine.current === next) { engine.current = null; setInstance(null); } }; }, [events]);
