...
export const GameCanvas = forwardRef<GameEngine, { events: EngineEvents; muted: boolean }>(({ events, muted }, ref) => {
  ...
  return <canvas ref={canvas} className="game-canvas" aria-label="Skybound Flap game area" onPointerDown={() => engine.current?.flap()} />;
});
