...
export class GameEngine {
  ...
  constructor(private canvas: HTMLCanvasElement, private events: EngineEvents, muted: boolean) { this.ctx = canvas.getContext('2d')!; this.audio = new GameAudio(muted); this.resize(); this.loop(0); }
  ...
  private reset() { this.score = 0; this.spawn = 0; this.particles = []; this.bird = { y: H / 2, velocity: 0, rotation: 0, wing: 0 }; this.pipes.forEach(p => p.active = false); }
  ...
  private update(dt: number) {
    ...
    const level = Math.floor(this.score / 10); const speed = 145 + level * 10; const gap = Math.max(128, 178 - level * 5);
    this.bird.velocity += 1120 * dt; this.bird.y += this.bird.velocity * dt; this.bird.rotation = clamp(this.bird.rotation + dt * 2.6, -.5, 1.15);
    this.spawn -= dt; if (this.spawn <= 0) { this.addPipe(gap); this.spawn = Math.max(1.08, 1.72 - level * .05); }
    for (const pipe of this.pipes) if (pipe.active) { pipe.x -= speed * dt; if (!pipe.counted && pipe.x + 64 < BIRD_X) { pipe.counted = true; this.score++; this.audio.score(); this.events.onScore(this.score); this.emit(); } if (pipe.x < -90) pipe.active = false; }
    if (this.bird.y - R < 0 || this.bird.y + R > H - GROUND || this.pipes.some(p => p.active && this.hitPipe(p))) this.die();
  }
  ...
  private draw() { const c = this.ctx; c.save(); c.clearRect(-20, -20, W + 40, H + 40); if (this.shake) c.translate((Math.random() - .5) * 9, (Math.random() - .5) * 9); this.background(c); for (const p of this.pipes) if (p.active) this.pipe(c, p); this.ground(c); this.particles.forEach(p => { c.globalAlpha = p.life; c.fillStyle = p.color; c.fillRect(p.x - 3, p.y - 3, 6, 6); }); this.birdDraw(c); c.restore(); }
  ...
  private birdDraw(c: CanvasRenderingContext2D) { const b = this.bird; c.save(); c.translate(BIRD_X, b.y); c.rotate(b.rotation); c.fillStyle = 'rgba(48,68,78,.2)'; c.beginPath(); c.ellipse(2, 24, 20, 5, 0, 0, Math.PI * 2); c.fill(); c.fillStyle = '#ffc94b'; c.beginPath(); c.arc(0, 0, R, 0, Math.PI * 2); c.fill(); c.fillStyle = '#f39a42'; c.beginPath(); c.ellipse(-4, 4 + Math.sin(b.wing) * 7, 13, 7, -.5, 0, Math.PI * 2); c.fill(); c.fillStyle = '#fff'; c.beginPath(); c.arc(7, -6, 6, 0, Math.PI * 2); c.fill(); c.fillStyle = '#24334d'; c.beginPath(); c.arc(9, -6, 2.5, 0, Math.PI * 2); c.fill(); c.fillStyle = '#ff7b48'; c.beginPath(); c.moveTo(16, 3); c.lineTo(29, 7); c.lineTo(16, 11); c.fill(); c.restore(); }
}
